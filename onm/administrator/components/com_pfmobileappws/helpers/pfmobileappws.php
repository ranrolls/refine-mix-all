...
class PfmobileappwsHelper
{
	...
	public static function addSubmenu($vName = 'notes')
	{
		...
		JSubMenuHelper::addEntry(
			JText::_('Notes'),
			'index.php?option=com_pfmobileappws&view=notes',
			($vName == 'notes')
		);	
	} else {
	...
	if(version_compare(JVERSION,'3','<')){
		JSubMenuHelper::addEntry(
			JText::_(''),
			'index.php?option=com_pfmobileappws&view=',
			($vName == '')
		);	
	} else {
		JHtmlSidebar::addEntry(
	...
	public static function getActions($categoryId = 0)
	{
		...
		if (empty($categoryId))
		{
			$assetName = 'com_pfmobileappws';
			$level = 'component';
		}
		else
		{
			$assetName = 'com_pfmobileappws.category.'.(int) $categoryId;
			$level = 'category';
		}
	
		$actions = JAccess::getActions('com_pfmobileappws', $level);
	...
	public static function getExtensions() 
	{
		...
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.'/components/com_pfmobileappws/elements/extensions.xml', 'JXMLElement');		        
	...
	public static function getCategoryViews()
	{
		...
		foreach($extensions as $extension ) {
			$views[$extension->listview->__toString()] = 'com_pfmobileappws.'.$extension->name->__toString();
		}
		return $views;
	}	
}
 ...
 * @since		1.5
 */
abstract class JHtmlPfmobileappws
{
	/**
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__pfmobileappws_categories AS a');
