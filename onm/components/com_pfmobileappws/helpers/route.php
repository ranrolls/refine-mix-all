...
/**
 * Pfmobileappws Component Route Helper
 *
 * @static
 * @package		Pfmobileappws
 * @subpackage	Helpers
 ...
 */
abstract class PfmobileappwsHelperRoute
{
	protected static $lookup;
	/**
	 * @param	int	The route of the pfmobileappws
	 */
	public static function getPfmobileappwsRoute($id, $catid)
	{
		$needles = array(
			'notes'  => array((int) $id)
		);
		//Create the link
		$link = 'index.php?option=com_pfmobileappws&view=notes&id='. $id;
		if ($catid > 1) {
			$categories = JCategories::getInstance('Notes');
		...
		if ($item = PfmobileappwsHelperRoute::_findItem($needles)) {
			$link .= '&Itemid='.$item;
		};
	...
	protected static function _findItem($needles = null, $identifier = 'id')
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component	= JComponentHelper::getComponent('com_pfmobileappws');
